...
// tongue-out: how wide the mouth has to be, relative to face width, to
// count as "open enough to be sticking your tongue out" rather than just
// talking. Watch the live debug HUD to tune this for your own face/lighting.
const TONGUE_OUT_MOUTH_OPEN_RATIO = 0.12;
...
function decideGesture(handResult) {
  ...
  if (!handResult.landmarks || handResult.landmarks.length === 0) {
    ...
    if (faceIsFresh && lastFace.mouthOpen > TONGUE_OUT_MOUTH_OPEN_RATIO) {
      return "tongueOut";
    }
  ...
  // 1. fist / punch: everything curled. A thumb stuck out from an otherwise
  // curled fist is a thumbs up, rather than a plain fist/punch.
  if (h.curledCount === 4) {
    if (h.thumbOut) {
      return "thumbsUp";
    }
...
function updateDebugHud() {
  ...
  debugHud.textContent =
    ...
    `mouthOpen: ${lastMouthOpenDebug.toFixed(2)}  (tongue-out thr ${TONGUE_OUT_MOUTH_OPEN_RATIO.toFixed(2)})`;
}
